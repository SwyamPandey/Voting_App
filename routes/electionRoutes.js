...
router.post('/', jwtAuthMiddleware, async (req, res) => {
  if (!(await checkAdmin(req.user.id)))
    return res.status(403).json({ message: 'Only admin can create election' });
...
// 3. Vote in election
router.post('/:id/vote/:candidateIndex', jwtAuthMiddleware, async (req, res) => {
  try {
    const election = await Election.findById(req.params.id);
    if (!election) return res.status(404).json({ message: 'Election not found' });
    ...
    if (user.role === 'admin') return res.status(403).json({ message: 'Admins are not allowed to vote' });

    if (user.votedElections && user.votedElections.includes(election._id)) {
...
//update an candidate
router.put('/:electionId/candidate/:candidateIndex', jwtAuthMiddleware, async (req, res) => {
  try {
    ...
    if (!user || user.role !== 'admin') {
      return res.status(403).json({ message: 'Only admin can update candidates' });
    }
...
//delete an candidate
router.delete('/:electionId/candidate/:candidateIndex', jwtAuthMiddleware, async (req, res) => {
  try {
    const { electionId, candidateIndex } = req.params;

    const user = await User.findById(req.user.id);
    if (!user || user.role !== 'admin') {
      return res.status(403).json({ message: 'Only admin can delete candidates' });
    }
